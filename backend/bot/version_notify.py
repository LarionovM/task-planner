...
CHANGELOGS: dict[str, str] = {
    ...
    "1.6.1": (
        "🔧 *Task Planner v1.6.1*\n\n"
        "🐛 *Исправлено*\n"
        "• Режим продуктивности — кнопка «Включить» теперь реально сохраняется\n"
        "• Бэклог — задачи со статусом «Готово» скрыты по умолчанию\n\n"
        "🎛 *Фильтр статусов в бэклоге*\n"
        "• Теперь можно выбрать несколько статусов одновременно\n"
        "• Toggle-кнопки вместо выпадающего списка\n"
    ),
    "1.6.0": (
        "🚀 *Task Planner v1.6.0*\n\n"
        "⚡ *Режим продуктивности*\n"
        "• Циклы фокуса включаются только когда ты этого хочешь\n"
        "• Переключатель в /settings и в настройках Web App\n\n"
        "🔧 *Исправлено*\n"
        "• Стоп и Пауза теперь реально останавливают уведомления\n"
        "• Регулярные задачи появляются в Календаре\n"
        "• После рестарта бот больше не спрашивает о задаче, которая ещё в процессе\n\n"
        "📅 *Регулярные задачи*\n"
        "• Можно указать время и длительность — задача автоматически попадает в слот\n\n"
        "🎨 *Дизайн*\n"
    ...
    "1.5.1": (
        ...
        "• Нерабочее время в календаре — кнопка «▼ нерабочее время» сворачивает секцию обратно\n"
    ),
    "1.5.0": (
        "🚀 *Task Planner v1.5.0*\n\n"
        "📅 *Нерабочее время в календаре*\n"
        "• Утренние и вечерние слоты теперь доступны — скрыты под кнопкой «▶ Утренние/Вечерние слоты»\n"
        "• Выходные дни: кнопка «▶ Показать слоты» открывает полную сетку\n"
        "• Раскрытие глобальное — время на одной строке во всех колонках\n"
        "• Состояние запоминается после перезагрузки\n"
