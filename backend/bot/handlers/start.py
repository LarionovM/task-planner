...
async def cmd_start(message: Message, allowed_user: AllowedUser):
    ...
    if is_first_launch:
        text = (
            ...
            "Я помогу тебе планировать день с помощью помодоро-техники.\n\n"
            "🔹 *Что я умею:*\n"
            "• 🍅 Автоматические помодоро-циклы весь рабочий день\n"
            "• 📋 Бэклог задач с приоритетами и статусами\n"
            "• 📅 Календарь с событиями (созвоны, встречи)\n"
            "• ⏰ Уведомления о старте и конце помодоро\n"
            "• 📊 Статистика по задачам и помодоро\n"
        ...
        text = (
            ...
            f"🍅 Помодоро: {user.pomodoro_work_min or 25} мин работа / "
